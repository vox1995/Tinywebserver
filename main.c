...
int main(int argc, char *argv[])
{
    ...
    printf("service running on localhost:%d\n", port);

    //忽略SIGPIPE信号
    signal(SIGPIPE, SIG_IGN);//addsig(SIGPIPE, SIG_IGN);

    //单例模式创建数据库连接池
    connection_pool *connPool = connection_pool::GetInstance("localhost", "root", "123456", "mydb", 3306, 5);
